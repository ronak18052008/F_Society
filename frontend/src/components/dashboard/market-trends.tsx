  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full bg-[var(--accent-forest-subtle)] px-3 py-1 text-xs font-semibold text-[var(--text-main)] border border-[var(--accent-forest)]/30">
        ...
        <Link
          href="/properties"
          className="inline-flex items-center gap-1.5 rounded-full bg-[var(--accent-forest)] hover:bg-[var(--accent-forest-hover)] px-4 py-2 text-xs font-semibold text-white shadow-xs hover:shadow-md transition cursor-pointer"
        >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="rounded-2xl border border-line bg-card p-5 shadow-card">
          ...
          <span className="text-[11px] text-[var(--success)] font-medium mt-0.5 block">
                ...
                <div key={item.city} className="group">
                  <div className="flex items-center justify-between text-xs mb-1">
                    <Link
                      ...
                      className="font-semibold text-ink group-hover:text-[var(--success)] transition flex items-center gap-1.5"
                    >
                      <span>{item.city}</span>
                  ...
                  <div className="h-2.5 w-full rounded-full bg-paper border border-line/40 overflow-hidden">
                    <div
                      className="h-full rounded-full bg-gradient-to-r from-[var(--accent-forest)] to-[var(--success)] transition-all duration-500"
                      style={{ width: `${pct}%` }}
                    />
                  </div>
                </div>
            ...
            <div className="space-y-3">
              {analytics.yearDistribution.map((yr) => (
                <div key={yr.year}>
                  ...
                  <div className="h-2.5 w-full rounded-full bg-paper border border-line/40 overflow-hidden">
                    <div
                      className="h-full rounded-full bg-[var(--accent-forest)]"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {analytics.bhkDistribution.map((b) => (
            <div key={b.bhk} className="rounded-2xl border border-line/60 bg-paper p-4 text-center">
              <span className="text-xs font-bold uppercase tracking-wider text-[var(--accent-forest)] block">
                {b.bhk}
              </span>
