              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {listed.map((item) => {
                  ...
                  return (
                    <div
                      key={item.id}
                      className="group rounded-3xl border border-[var(--border)] bg-card overflow-hidden shadow-xs hover:shadow-card hover:border-[var(--primary-pista)]/50 transition-all flex flex-col justify-between"
                    >
                      <div>
                        ...
                        <div className="relative aspect-[16/10] w-full overflow-hidden bg-paper">
                          ...
                          <div className="absolute left-3.5 top-3.5 z-10 flex items-center gap-2">
                            <span className="inline-flex items-center gap-1.5 rounded-full bg-black/70 backdrop-blur-md px-2.5 py-1 text-[11px] font-semibold text-white border border-white/15 shadow-xs">
                              <span>📷</span>
                              <span>{images.length} Photo{images.length > 1 ? "s" : ""}</span>
                            </span>
                            <span className="rounded-full bg-emerald-500/90 backdrop-blur-md px-2.5 py-1 text-[10px] font-bold text-white uppercase shadow-xs">
                              Live &amp; Active
                            </span>
                          </div>

                          <div className="absolute bottom-3 left-3.5 z-10">
                            <span className="inline-flex items-center gap-1.5 rounded-full bg-black/75 backdrop-blur-md px-3 py-1 text-[11px] font-medium text-white">
                              ...
                              <span className="truncate max-w-[200px]">{item.locality}, {item.city}</span>
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="p-5 space-y-3">
                          <div className="flex items-start justify-between gap-2">
                            <Link
                              ...
                              className="text-base font-bold text-ink hover:text-[var(--accent-forest)] transition-colors line-clamp-1"
                            >
                              {item.title}
                            </Link>
                            <span className="text-sm font-bold text-ink font-serif whitespace-nowrap">
                              {formatInr(item.rent)}<span className="text-xs font-sans text-ink-muted font-normal">/mo</span>
                            </span>
                          </div>

                          {/* Specifications Pills */}
                          <div className="flex flex-wrap items-center gap-1.5 text-xs text-ink-muted">
                            <span className="rounded-lg bg-paper border border-line px-2.5 py-1 font-semibold text-ink">
                              {item.bedrooms || item.bhk || 1} BHK
                            </span>
                            <span className="rounded-lg bg-paper border border-line px-2.5 py-1 font-medium font-tabular text-ink">
                              {item.areaSqft || item.sizeSqft || 650} sqft
                            </span>
                            <span className="rounded-lg bg-paper border border-line px-2.5 py-1 font-medium capitalize text-ink">
                              {item.furnishingStatus || item.furnishing || "Semi-Furnished"}
                            </span>
                            {item.tenantPreferred && (
                              <span className="rounded-lg bg-[var(--primary-pista)]/15 border border-[var(--primary-pista)]/30 px-2 py-0.5 text-[11px] font-medium text-[var(--accent-forest)]">
                                {item.tenantPreferred}
                              </span>
                            )}
                          </div>
                      ...
                      {/* Action Footer */}
                      <div className="px-5 pb-5 pt-3 border-t border-[var(--border)] flex items-center justify-between gap-3 text-xs">
                        <Link
                          href={`/owner/properties/${item.id}`}
                          className="inline-flex items-center gap-1.5 rounded-full bg-[var(--accent-forest)] hover:bg-[var(--accent-forest-hover)] text-white px-4 py-2 font-semibold shadow-xs transition-colors cursor-pointer"
                        >
                        ...
                        <div className="flex items-center gap-3">
                          <Link
                            ...
                            target="_blank"
                            className="font-semibold text-ink-muted hover:text-ink flex items-center gap-1"
                          >
