...
export default function DocumentsPage() {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-2 pb-6 border-b border-warm-200/80 dark:border-forest/40">
        {[
          ...
          <button
            ...
            className={`rounded-full px-4 py-1.5 text-xs font-semibold transition-all cursor-pointer ${
              category === item.value
                ...
                : "border border-warm-200 dark:border-forest/40 bg-card/70 dark:bg-card-dark/70 text-ink-muted hover:text-ink dark:hover:text-cream"
            }`}
          >
            {item.label}
          </button>
      ...
      <div className="mt-8 max-w-xl rounded-2xl border border-warm-200/80 dark:border-forest/40 bg-card dark:bg-card-dark p-6 shadow-xs">
        <h3 className="text-base font-serif font-bold text-ink dark:text-cream mb-2">Upload Tenancy Document</h3>
        <UploadField
      ...
      {/* Documents Grid */}
      {visible.length === 0 ? (
        <div className="mt-10 rounded-2xl border border-dashed border-warm-300 dark:border-forest/40 p-10 text-center bg-card/40 dark:bg-card-dark/30">
          <EmptyState
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2">
          {visible.map((doc) => (
            <article
              key={doc.id}
              className="rounded-2xl border border-warm-200/80 dark:border-forest/40 bg-card dark:bg-card-dark p-5 shadow-xs hover:shadow-md hover:border-pista transition-all flex flex-col justify-between"
            >
              <div>
                ...
                <div className="mt-3 flex items-start gap-3">
                  ...
                  <div>
                    <h2 className="text-base font-serif font-bold text-ink dark:text-cream leading-tight">{doc.title}</h2>
                    <p className="text-xs text-ink-muted mt-0.5 truncate max-w-xs">{doc.fileName}</p>
                  </div>
                </div>
              </div>

              <div className="mt-5 pt-3 border-t border-warm-200/60 dark:border-forest/30 flex items-center justify-between text-xs">
                <span className="text-ink-muted">{formatDateTime(doc.uploadedAt)}</span>
                <button
                  type="button"
                  onClick={() => handleDownload(doc)}
                  className="inline-flex items-center gap-1.5 font-semibold text-forest dark:text-pista hover:underline cursor-pointer"
                >
